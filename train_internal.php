...
$venv_path = getenv('HOME') . '/.yolov11_venv';
$activate = "$venv_path/bin/activate";

if (!is_dir($venv_path)) {
    echo "   Creating virtualenv at $venv_path...\n";
    flush();
    system("python3 -m venv " . escapeshellarg($venv_path));
    system("source " . escapeshellarg($activate) . " && pip install ultralytics onnx2tf tf_keras onnx_graphsurgeon sng4onnx 2>&1");
}

$imgsz = $GLOBALS["imgsz"] ?? 800;
$train_cmd = "source " . escapeshellarg($activate) . " && "
    . "cd " . escapeshellarg($tmp_dir) . " && "
    . "yolo detect train"
...
$process = proc_open("bash -c " . escapeshellarg($train_cmd), $descriptorspec, $pipes);
